...
void PushToReqQueue(P_LOW_LEVEL_REQ_INFO lowLevelCmd)
{
	...
	{
		unsigned int tempLun = lowLevelCmd->rowAddr / PAGE_NUM_PER_LUN;
...
{
	int rear;
	unsigned int phyPage, phyRowAddr;
	unsigned int tempLun = rowAddress / PAGE_NUM_PER_LUN;
	...
	switch(dieStatusTable->dieStatusEntry[chNo][wayNo].dieStatus)
	{
		...
		case DS_EXE:
			...
			else if(reqStatus == RS_WARNING)
			{
				...
				blockNo = tempLun * BLOCK_NUM_PER_LUN + tempRowAddr / PAGE_NUM_PER_BLOCK_MLC;

				xil_printf("RS_WARNING - bad block manage [chNo %x wayNo %x Block %x Rowaddr %x]\r\n",chNo, wayNo, blockNo, reqQueue->reqEntry[front][chNo][wayNo].rowAddr);
...
void EmptyLowLevelQ(int firstQueue)
{
	...
	if(badBlockUpdate)
	{
		...
		diePpn = metadataBlockNo * PAGE_NUM_PER_BLOCK + 1; //start at second page
		...
		while (loop > 0)
		{
			PushToSubReqQueue(chNo, wayNo, V2FCommand_ReadPageTrigger, diePpn, tempBuffer, SPARE_ADDR);
		...
		for(entry = 0; entry < REQ_QUEUE_DEPTH; ++entry)
			for(wayNo = 0; wayNo <WAY_NUM; ++wayNo)
				for(chNo = 0; chNo < CHANNEL_NUM; ++chNo)
				{
					if(newBadBlockTable->newBadBlockEntry[entry][chNo][wayNo] != 0xffffffff)
					{
						shifter = (unsigned char*)(GC_BUFFER_ADDR + newBadBlockTable->newBadBlockEntry[entry][chNo][wayNo] + (wayNo * CHANNEL_NUM + chNo) *BLOCK_NUM_PER_DIE);
						*shifter = 1;
		...
		while (loop>0)
		{
			PushToSubReqQueue(chNo, wayNo, V2FCommand_ProgramPage, diePpn, tempBuffer, SPARE_ADDR);
