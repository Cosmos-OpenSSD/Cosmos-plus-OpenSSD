...
void InitFtlMapTable()
{
	...
	InitPageMap();
	InitBlockMap(GC_BUFFER_ADDR, (GC_BUFFER_ADDR + BLOCK_NUM_PER_SSD));
	InitDieBlock();
	InitGcMap();

	storageCapacity_L = (SSD_SIZE - (FREE_BLOCK_SIZE + DIE_NUM * METADATA_BLOCK_SIZE + badBlockSize + OVER_PROVISION_BLOCK_SIZE)) * ((1024*1024) / SECTOR_SIZE_FTL);
...
void InitDieStatusTable()
{
	...
	completeTable0 = (struct completeArray*)COMPLETE_TABLE_ADDR0;
	errorInfoTable0 = (struct errorInfoArray*)ERROR_INFO_TABLE_ADDR0;
	completeTable1 = (struct completeArray*)COMPLETE_TABLE_ADDR1;
	errorInfoTable1 = (struct errorInfoArray*)ERROR_INFO_TABLE_ADDR1;
	...
	for(chNo=0; chNo<CHANNEL_NUM; ++chNo)
	{
		...
		for(wayNo=0; wayNo<WAY_NUM; ++wayNo)
		{
			...
			completeTable0->completeEntry[chNo][wayNo] = 0;
			completeTable1->completeEntry[chNo][wayNo] = 0;
