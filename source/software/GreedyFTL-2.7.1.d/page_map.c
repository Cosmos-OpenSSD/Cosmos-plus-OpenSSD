...
//
// * v1.6.0
//	 - Read buffer size used to read a raw page is revised in recognizing bad block
//
// * v1.5.0
	...
	if(checkBadBlock)
	{
		for(phyBlockNo = 0; phyBlockNo < realBlockNoPerDie; phyBlockNo++)
		{
			for(dieNo=0; dieNo < DIE_NUM; dieNo++)
				...
				{
					tempBadMark[dieNo] = 0;
					tempBuffer =  readBufAddr + DIE_NUM * (realBlockNoPerDie / PAGE_SIZE + 1) * PAGE_SIZE + dieNo*BYTE_PER_ROW;
			...
			for(dieNo=0; dieNo < DIE_NUM; dieNo++)
				if(!badBlockTableExist[dieNo])
				{
					markPointer1 = (unsigned char*)(readBufAddr + DIE_NUM * (realBlockNoPerDie / PAGE_SIZE + 1) * PAGE_SIZE + dieNo*BYTE_PER_ROW + BAD_BLOCK_MARK_LOCATION1);
					markPointer2 = (unsigned char*)(readBufAddr + DIE_NUM * (realBlockNoPerDie / PAGE_SIZE + 1) * PAGE_SIZE + dieNo*BYTE_PER_ROW + BAD_BLOCK_MARK_LOCATION2);
